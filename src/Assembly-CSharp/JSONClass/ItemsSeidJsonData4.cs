...
public class ItemsSeidJsonData4 : IJSONClass
{
	...
	{
		foreach (JSONObject item in jsonData.instance.ItemsSeidJsonData[4].list)
		{
			try
			{
				...
				{
					PreloadManager.LogException($"!!!错误!!!向字典ItemsSeidJsonData4.DataDict添加数据时出现重复的键，Key:{itemsSeidJsonData.id}，已跳过，请检查配表");
					continue;
				}
