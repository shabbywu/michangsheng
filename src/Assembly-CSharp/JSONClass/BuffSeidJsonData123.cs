...
public class BuffSeidJsonData123 : IJSONClass
{
	...
	{
		foreach (JSONObject item in jsonData.instance.BuffSeidJsonData[123].list)
		{
			try
			{
				...
				{
					PreloadManager.LogException($"!!!错误!!!向字典BuffSeidJsonData123.DataDict添加数据时出现重复的键，Key:{buffSeidJsonData.id}，已跳过，请检查配表");
					continue;
				}
