...
public class CrateAvatarSeidJsonData4 : IJSONClass
{
	...
	{
		foreach (JSONObject item in jsonData.instance.CrateAvatarSeidJsonData[4].list)
		{
			try
			{
				...
				{
					PreloadManager.LogException($"!!!错误!!!向字典CrateAvatarSeidJsonData4.DataDict添加数据时出现重复的键，Key:{crateAvatarSeidJsonData.id}，已跳过，请检查配表");
					continue;
				}
