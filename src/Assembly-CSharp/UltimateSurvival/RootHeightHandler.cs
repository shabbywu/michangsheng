...
public class RootHeightHandler : PlayerBehaviour
{
	...
	private void Start()
	{
		...
		base.Player.Crouch.AddStartListener(OnStart_Crouch);
		base.Player.Crouch.AddStopListener(OnStop_Crouch);
		m_InitialHeight = ((Component)this).transform.localPosition.y;
	}

	private void OnStart_Crouch()
	{
		((MonoBehaviour)this).StopAllCoroutines();
		((MonoBehaviour)this).StartCoroutine(SetOffsetOnY(m_CrouchOffset));
	}

	private void OnStop_Crouch()
	{
		((MonoBehaviour)this).StopAllCoroutines();
		((MonoBehaviour)this).StartCoroutine(SetOffsetOnY(0f));
	}
	...
	private IEnumerator SetOffsetOnY(float targetY)
	{
		...
		while (Mathf.Abs(targetY - m_CurrentOffsetOnY) > Mathf.Epsilon)
		{
			...
			((Component)this).transform.localPosition = Vector3.up * (m_CurrentOffsetOnY + m_InitialHeight);
