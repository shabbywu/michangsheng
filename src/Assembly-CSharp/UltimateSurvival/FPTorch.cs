...
public class FPTorch : FPObject
{
	...
	public override void On_Draw(SavableItem correspondingItem)
	{
		...
		((MonoBehaviour)this).StartCoroutine(C_ToggleTorch(toggle: true));
	}
	...
	public override void On_Holster()
	{
		...
		{
			((MonoBehaviour)this).StopAllCoroutines();
			((MonoBehaviour)this).StartCoroutine(C_ToggleTorch(toggle: false));
		}
	}
	...
	private IEnumerator C_ToggleTorch(bool toggle)
	{
		...
		{
			m_Light.intensity = Mathf.MoveTowards(m_Light.intensity, toggle ? m_LightIntensity : 0f, Time.deltaTime * m_LightIntensity / m_LightDrawDuration);
			m_Light.range = Mathf.MoveTowards(m_Light.range, toggle ? m_LightRange : 0f, Time.deltaTime * m_LightIntensity / m_LightDrawDuration);
			yield return null;
		}
