...
public class Firelight : MonoBehaviour
{
	...
	public void Toggle(bool toggle)
	{
		...
		((MonoBehaviour)this).StartCoroutine(C_Toggle(toggle));
	}
	...
	private IEnumerator C_Toggle(bool toggle)
	{
		...
		{
			m_Light.intensity = Mathf.MoveTowards(m_Light.intensity, toggle ? m_LightIntensity : 0f, Time.deltaTime * m_LightIntensity / m_ToggleDuration);
			m_Light.range = Mathf.MoveTowards(m_Light.range, toggle ? m_LightRange : 0f, Time.deltaTime * m_LightIntensity / m_ToggleDuration);
			yield return null;
		}
