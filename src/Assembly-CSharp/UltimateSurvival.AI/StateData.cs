...
public class StateData
{
	...
	{
		OverrideValue(new KeyValuePair<string, object>(key, value), data);
	}

	public static void OverrideValues(StateData overrider, StateData data)
	{
		foreach (KeyValuePair<string, object> item in overrider.m_Dictionary)
		{
			data.m_Dictionary[item.Key] = item.Value;
		}
	}
}
